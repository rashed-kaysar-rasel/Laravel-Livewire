        ...
        <table class="min-w-full bg-white dark:bg-gray-800">
            ...
            <tbody>
                @foreach($users as $user)
                    <tr wire:key="{{$user->id}}" class="bg-gray-100 dark:bg-gray-900 border-b dark:border-gray-700">
                        <td class="px-4 py-2">{{ $user->name }}</td>
                        <td class="px-4 py-2">{{ $user->email }}</td>
                        <td class="px-4 py-2 text-center">
                            <button class="text-blue-500 hover:text-blue-700" onclick="Livewire.dispatch('openModal', { component: 'user.edit-user', arguments: { user: {{ $user->id }} }})">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                    <path d="M17.414 2.586a2 2 0 00-2.828 0L6 11.172V14h2.828l8.586-8.586a2 2 0 000-2.828zM4 12V15h3l-3-3z" />
                                </svg>
                            </button>
                            <button onclick="confirm('Are you sure you want to delete {{$user->name}} ?') ? '' : event.stopImmediatePropagation()" class="text-red-500 hover:text-red-700" wire:click="delete({{ $user->id }})">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
